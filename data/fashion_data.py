...
class Dataset(Dataset):
    def __init__(self, opt, is_inference, labels_required = False):
        ...
        self.data = self.get_paths(self.root, self.file_path) # List[dict]. dict:['source_image', 'source_label', 'target_image', 'target_label'] 
    ...
    def get_paths(self, root, path):
        ...
        for item in lines:
            ...
            dict_item['source_label'] = [os.path.join(self.semantic_path, self.img_to_label(path)) for path in dict_item['source_image']] # source_image pose coordinates.
