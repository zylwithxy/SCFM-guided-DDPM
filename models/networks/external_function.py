...
class MultiAffineRegularizationLoss(nn.Module):
    def __init__(self, kz_list: list): # kz_dic= {'2': 5, '3': 3}
        super(MultiAffineRegularizationLoss, self).__init__()
        temp = {'3': 3, '4': 2, '5': 2}
        kz_dic = {str(key): temp[str(key)] for key in kz_list}
        self.kz_dic=kz_dic
        self.method_dic={}
        for key in kz_dic:
...
class PerceptualCorrectness(nn.Module):
    ...
    def __init__(self, layer=['rel1_1','relu2_1','relu3_1','relu4_1', 'relu5_1', 'relu5_4']):
    ...
    def __call__(self, target, source, flow_list, used_layers, unetautoenc, mask=None, use_bilinear_sampling=False):
        used_layers=sorted(used_layers, reverse=True)
        # self.target=target
        # self.source=source
        self.target_vgg, self.source_vgg = self.vgg(target), self.vgg(source)
        # target_vgg, source_vgg = unetautoenc.encode(target)['cond'], unetautoenc.encode(source)['cond']
        # self.target_vgg, self.source_vgg = [f_map.detach() for f_map in target_vgg], [f_map.detach() for f_map in source_vgg]
        ...
        #     loss += self.calculate_loss(flow_list[i], used_layers[i], mask, use_bilinear_sampling)
