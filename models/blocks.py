...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    """
    def forward(self, x, emb=None, cond=None, lateral=None, **kwargs):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb=emb, cond=None, lateral=lateral)
            elif isinstance(layer, AttentionBlock):
                x = layer(x, cond, **kwargs)
            else:
                x = layer(x)
        return x
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        if conf is not None:
            self.use_attention_qkv = conf.use_attention_qkv
        
        self.use_checkpoint = use_checkpoint
        if hasattr(self, 'use_attention_qkv') and self.use_attention_qkv:
        ...
        self.target = PoseTargetConfig(attn_layer=[3], 
                                       extractor_kz = {'3': 3}, use_encoder= False, use_flowdisp= conf.use_flowdisp,
                                       flowdisp_inputdim= self.channels,
                                       resolution= resolution).make_model()

    def forward(self, x, cond, flow_fields: th.Tensor = None, masks: th.Tensor = None, prob= 1):
        return torch_checkpoint(self._forward, (x, cond, flow_fields, masks, prob), self.use_checkpoint)

    def _forward(self, x, cond, flow_fields: th.Tensor, masks: th.Tensor, prob):
        ...
        # conf.sample_method == 'pose_target_wo_target_encoder'
        if hasattr(self, 'target') and prob > 0:
            cond_warped = self.target(x, [cond], [flow_fields], [masks]) # spatial dimensions of these 4 inputs are the same
            cond = cond_warped[0]

        if hasattr(self, 'target') and not self.use_attention_qkv and prob > 0:
            # return cond
            return x + cond
        elif hasattr(self, 'target') and not self.use_attention_qkv and prob == 0:
            return x
