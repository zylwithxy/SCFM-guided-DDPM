...
class BeatGANsUNetConfig(BaseConfig):
    ...
    output_omit_block_id: Tuple[int] = (0, 1)
...
class BeatGANsUNetModel(nn.Module):
    def __init__(self, conf: BeatGANsUNetConfig):
        ...
        if conf.enable_archi_inres_remove:
            assert conf.enable_architecture_remove, "Enable architecture_remove before archi_inres_remove"
        ...
        for level, mult in enumerate(conf.input_channel_mult
            ...
            for block_id in range(conf.num_input_res_blocks or conf.num_res_blocks):
                ...
                if conf.enable_archi_inres_remove and level == 4 and block_id == 1:
                    ch = 256
                layers = [
                    ResBlockConfig(
                        ch,
                ...
                if level in [0, 1, 2] and block_id in [0, 1] and conf.enable_architecture_remove:
                    if level == 2 and block_id == 0:
                        self.input_blocks.append(TimestepEmbedSequential(*layers))
                    else:
                        self.input_blocks.append(None)
                elif level in [3, 4, 5] and block_id == 0 and conf.enable_archi_inres_remove: # for res in [32, 16, 8] 
                    self.input_blocks.append(None)
                else:
                    self.input_blocks.append(TimestepEmbedSequential(*layers))
                self._feature_size += ch
        ...
        for level, mult in list(enumerate(conf.channel_mult))[::-1]:
            for i in range(conf.num_res_blocks + 1):
                ...
                try:
                    ...
                    ich = 0
                # print('pop:', ich)
                if level in [0, 1, 2] and i in conf.output_omit_block_id:
                    layers = [
                    ResBlockConfig(
                ...
                elif level in [3, 4, 5] and i == 1 and conf.enable_archi_inres_remove:
                    layers = [
                    ResBlockConfig(
                        # only direct channels when gated
                        channels=ch,
                ...
                # self.output_blocks.append(None if conf.enable_architecture_remove and level in [0, 1, 2] and i in conf.output_omit_block_id
                                            # else TimestepEmbedSequential(*layers))

                if conf.enable_architecture_remove and level in [0, 1, 2] and i in conf.output_omit_block_id:
                    self.output_blocks.append(None)
                elif conf.enable_archi_inres_remove and level in [3, 4, 5] and i == 1:
                    self.output_blocks.append(None)
                else:
                    self.output_blocks.append(TimestepEmbedSequential(*layers))
                
                if level == 1 and i == 0 and conf.enable_architecture_remove:
                    del self.output_blocks[12]
                    self.output_blocks.append(TimestepEmbedSequential(*layers))

                # if level == 0 and i == 2:
                #     del self.output_blocks[17] # For the consideration of dimension
                #     self.output_blocks.append(TimestepEmbedSequential(*layers))
...
class BeatGANsEncoder(nn.Module):
    def __init__(self, conf: BeatGANsUNetConfig):
        ...
        if conf.enable_source_enc_inres_remove:
            assert conf.enable_source_enc_remove, "Enable source_enc_remove before source_enc_inres_remove"
        ...
        for level, mult in enumerate(conf.input_channel_mult
                                     or conf.channel_mult):
            for block_id in range(conf.num_input_res_blocks or conf.num_res_blocks):
                if conf.enable_source_enc_inres_remove and level == 4 and block_id == 1:
                    ch = 256
                layers = [
                    ResBlockConfig(
                        ch,
                ...
                #             use_new_attention_order,
                #         ))
                if conf.enable_source_enc_remove and level in [0, 1, 2] and block_id in [0, 1]:
                    self.input_blocks.append(TimestepEmbedSequential(*layers) 
                                             if level == 2 and block_id == 0 else None)
                elif conf.enable_source_enc_inres_remove and level in [3, 4, 5] and block_id == 0: # for res in [32, 16, 8] 
                    self.input_blocks.append(None)
                else:
                    self.input_blocks.append(TimestepEmbedSequential(*layers))
    ...
    def forward(self, x, t=None, y=None, **kwargs):
        ...
        for i in range(len(self.input_num_blocks)):
            for j in range(self.input_num_blocks[i]):
                ...
                results.append(h if self.input_blocks[k] is not None else None)
                #print (h.shape)
                k += 1
        assert k == len(self.input_blocks)
