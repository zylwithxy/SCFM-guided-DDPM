...
import torch
import tqdm
from models.networks import external_function
...
def ddim_steps(x, seq, model, b, x_cond, diffusion = None, **kwargs):
    # x_cond = [val_img, val_pose] val_pose.shape[0] == 2 * val_img.shape[0]
    val_img = x_cond[0]
    ...
    with torch.no_grad():
        ...
        xs = [x]
        xt = x
        for idx, (i, j) in enumerate(tqdm.tqdm(zip(reversed(seq), reversed(seq_next)))):
            t = (torch.ones(n) * i).to(x.device)
            next_t = (torch.ones(n) * j).to(x.device)
            at = compute_alpha(b, t.long())
            at_next = compute_alpha(b, next_t.long())

            if model.conf.use_warp:
                [cond, target_pose] = x_cond[:2]
                target_B, source_B = torch.chunk(target_pose, 2, dim= 0)
                # source_pose = torch.cat([source_B, target_B], dim= 0)
                flow_fields, masks = model.flow_net(val_img, source_B, target_B) 
            else:
                [cond, target_pose] = x_cond[:2]

                if target_pose.shape[0] != cond[0][0].shape[0]:
                    target_B, source_B = torch.chunk(target_pose, 2, dim= 0)
                else:
                    target_B = target_pose
                flow_fields = None
                masks = None

            results_cond = model.forward_with_cond_scale(x = torch.cat([xt, target_B],1), t = t, cond = cond, cond_scale = 2, flow_fields= flow_fields, masks= masks, ddim_step= idx)   
            et, flow_fields, feature_map_out = results_cond[0], results_cond[-2], results_cond[-1]
    ...
    return [xt], x0_preds, flow_fields, feature_map_out
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.Regularization = external_function.MultiAffineRegularizationLoss(kz_list= attn_layer).cuda()
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        :param t: the number of diffusion steps (minus 1). Here, 0 means one step. (t + 1) means the true time step.
    ...
    def p_sample_loop_progressive(
        ...
        if progress:
            # Lazy import so that we don't depend on tqdm. which means more faster due to importing related dependencies.
    ...
    def _vb_terms_bpd(
        ...
        # out["mean"]: torch.Size([4, 3, 256, 256]), out["log_variance"]: torch.Size([4, 3, 256, 256])(broadcast)

        kl = mean_flat(kl) / np.log(2.0) # torch.Size([4])
        # For x_t, t != 0
        ...
        decoder_nll = mean_flat(decoder_nll) / np.log(2.0) # torch.Size([4])
        # For x_t, t == 0
    ...
    def training_losses(self, model, x_start, cond_input, t, prob, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            # x_t.shape: [4, 3, 256, 256]; target_pose.shape: [4, 20, 256, 256]
            model_output, flow_fields, feature_map_out = model(x = torch.cat([x_t, target_pose],1), t = self._scale_timesteps(t), x_cond = img, prob = prob) # torch.Size([4, 6, 256, 256])

            if self.model_var_type in [
                ModelVarType.LEARNED,
                ModelVarType.LEARNED_RANGE,
            ]:
                B, C = x_t.shape[:2] # B= 4; 3
                ...
                terms["vb"] = self._vb_terms_bpd(
                    model=lambda *args, r=frozen_out: r, # lambda expression with default parameters: lambda argument1=default_value1, argument2=default_value2 : expression
                    x_start=x_start,
                    x_t=x_t,
                    t=t,
                    clip_denoised=False,
                )["output"] # terms["vb"].shape == torch.Size([4])
            ...
            }[self.model_mean_type] # noise torch.Size([4, 3, 256, 256])
            ...
            # Add warp loss
            if model.module.conf.use_warp:
                # Calculate Sampling Correctness Loss
                lambda_correct = 1.0
                # import pdb; pdb.set_trace()
                # loss_correctness_gen = self.Correctness(x_start, img, flow_fields[1:], model.module.flow_net.attn_layer[:1]) * lambda_correct
                # loss_correctness_gen = self.Correctness(x_start, img, flow_fields, model.module.flow_net.attn_layer) * lambda_correct
                # loss_correctness_gen = self.Correctness(x_start, img, flow_fields, [10, 11], model.module) * lambda_correct
                attn_layer = model.module.flow_net.attn_layer * 2 if model.module.conf.use_warp_separate else model.module.flow_net.attn_layer
                loss_correctness_gen = self.Correctness(x_start, img, flow_fields, attn_layer, None) * lambda_correct
                terms['correctness_gen'] = loss_correctness_gen # mean

                # Calculate regularization term
                lambda_regularization = 0.0025
                loss_regularization = self.Regularization(flow_fields) * lambda_regularization
                terms['regularization'] = loss_regularization # mean
