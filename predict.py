...
class Predictor():
    def __init__(self, ckpt_path, args):
        ...
        if args.use_warp:
            ...
            # config.output_omit_block_id = (1, 2)
            # self.model = get_model_conf().make_model()
            self.model = config.make_model()
        else:
            raise PermissionError(f"This branch should use optical flow, but the model config of use_warp is {args.use_warp}")
        
        # ckpt = torch.load("checkpoints/last.pt")
        ckpt = torch.load(ckpt_path)
        self.model.load_state_dict(ckpt["ema"]) # due to the 'ema' on only one cuda device; model is paralleled
        self.model = self.model.cuda()
        self.model.eval()

        self.betas = conf.diffusion.beta_schedule.make()
        self.diffusion = create_gaussian_diffusion(self.betas, attn_layer= get_model_conf().flownet_attnlayer, predict_xstart = False)#.to(device)
