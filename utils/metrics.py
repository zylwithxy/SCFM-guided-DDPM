...
class FID():
    ...
    def compute_statistics_of_path(self, path, verbose):
        ...
        if os.path.exists(npz_file):
            ...
            description = 'read gt_train imgs' if len(files) > 30000 else 'read generated imgs'
            # itemsize = imread(str(files[0])).size * imread(str(files[0])).itemsize
            # import pdb; pdb.set_trace()
            imgs = np.array([(cv2.resize(imread(str(fn)).astype(np.float32),(176, 256))) for fn in tqdm(files, desc= description)])#np.array([imread(str(fn)).astype(np.float32) for fn in files])
...
def preprocess_path_for_deform_task(gt_path, distorted_path):
    ...
    for distorted_image in distorted_image_list:
        ...
        gt_image = os.path.join(gt_path, image)
        if not os.path.isfile(gt_image):
            raise FileNotFoundError(f"Image not found in test set: {image}")
        gt_list.append(gt_image)
        distorated_list.append(distorted_image)    
...
class LPIPS():
    ...
    def calculate_from_disk(self, path_1, path_2, batch_size=64, verbose=False, sort=True):
        ...
        for i in range(n_batches):
            ...
            imgs_1 = np.array([cv2.resize(imread(str(fn)).astype(np.float32),(176, 256))/127.5-1 for fn in tqdm(files_1[start:end], desc= 'lpips_generated_img_read')])
            imgs_2 = np.array([cv2.resize(imread(str(fn)).astype(np.float32),(176, 256))/127.5-1 for fn in tqdm(files_2[start:end], desc= 'lpips_gt_img_read')])
            ...
            result.append(self.model.forward(img_1_batch, img_2_batch).detach().cpu())


        distance = torch.cat(result,0)[:,0,0,0].mean().item()
...
if __name__ == "__main__":
    ...
    fid_lpips = {'fid': FID, 'lpips': LPIPS}
    REC.update(fid_lpips)

    sav_dir = os.path.join(args.name, os.path.basename(args.distorated_path))
    
    if not os.path.exists(sav_dir):
        os.makedirs(sav_dir)

    distorated_len = len([file for file in os.listdir(distorated_path) if file.endswith('.png')])
    file_name = os.path.join(sav_dir, f'metrics_{distorated_len}.txt')
    
    time_struct = time.localtime(time.time())
    formatted_time = time.strftime("%Y-%m-%d, %H:%M:%S", time_struct)

    with open(file_name, 'at') as opt_file:
        opt_file.write('------------ Metrics -------------\n')
        opt_file.write(f'{formatted_time}\n')
